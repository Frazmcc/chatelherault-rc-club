...
async function addUsage(kv, monthKey, bytes) {
  ...
  await kv.put(`metoffice:usage:${monthKey}`, String(updated))
}
...
export async function onRequestGet(context) {
  ...
  if (!upstreamResponse.ok) {
    ...
    return new Response(
      JSON.stringify({
        error: `Met Office upstream request failed (${upstreamResponse.status}).`,
        details,
      }),
      {
  ...
  await addUsage(usageKv, monthKey, payloadBytes)
